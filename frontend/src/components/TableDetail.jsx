...
import { BiSolidTrash, BiPrinter  } from "react-icons/bi";
...
const TableDetail = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setDetail(response.data[0].payload);
    } catch (error) {
  ...
  const deleteDetail = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/detail/${id}`);
      fetchData();  
    } catch (error) {
    ...
    <div className="relative w-5/6 mx-auto mt-[50px]">
      <div className="text-right ">
        <div className="bg-cyan-500 px-4 py-2 mb-4 shadow-xl rounded-xl font-medium inline-block">
          <NavLink to="/detail/create">Tambah Data</NavLink>
        </div>
      </div>
            ...
            {currentRows.map((data) => (
              ...
              >
                <td className="py-4 px-2 flex items-center justify-center gap-3">
                  ...
                  />
                  <NavLink to={`/detail/${data.id}/print`}>
                    <BiPrinter  className=" bg-green-500 py-1 px-1.5 rounded cursor-pointer text-3xl text-netral-900" />
                  </NavLink>
                </td>
